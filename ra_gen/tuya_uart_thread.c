...
void tuya_uart_thread_create(void);
static void tuya_uart_thread_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t tuya_uart_thread_parameters =
{ .p_context = (void*) NULL, };
...
#if 1
    tuya_uart_thread = xTaskCreateStatic (
#else
                    BaseType_t tuya_uart_thread_create_err = xTaskCreate(
                                          ...
                                          3,
#if 1
                                          (StackType_t*) &tuya_uart_thread_stack,
                                          (StaticTask_t*) &tuya_uart_thread_memory
#else
                        & tuya_uart_thread
                        #endif
                                          );
...
#if 1
    if (NULL == tuya_uart_thread)
    {
        rtos_startup_err_callback (tuya_uart_thread, 0);
    }
#else
                    if (pdPASS != tuya_uart_thread_create_err)
                    {
                        rtos_startup_err_callback(tuya_uart_thread, 0);
                    }
                    #endif
...
static void tuya_uart_thread_func(void *pvParameters)
{
    ...
    tuya_uart_thread_entry (pvParameters);
}
