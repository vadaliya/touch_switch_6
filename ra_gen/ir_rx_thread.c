...
void ir_rx_thread_create(void);
static void ir_rx_thread_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t ir_rx_thread_parameters =
{ .p_context = (void*) NULL, };
...
#if 1
    ir_rx_thread = xTaskCreateStatic (
#else
...
#if 1
    if (NULL == ir_rx_thread)
    {
        rtos_startup_err_callback (ir_rx_thread, 0);
    }
#else
                    if (pdPASS != ir_rx_thread_create_err)
                    {
                        rtos_startup_err_callback(ir_rx_thread, 0);
                    }
                    #endif
...
static void ir_rx_thread_func(void *pvParameters)
{
    ...
    ir_rx_thread_entry (pvParameters);
}
